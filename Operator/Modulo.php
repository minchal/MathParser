...
class Operator_Modulo extends Operator {
	...
	public function group($tokens) {
		while ( ($key = self::hasToken($tokens, '%')) !== false ) {
			$value = new Value_Modulo();
			
			if (!isset($tokens[$key+1])) {
				throw new Parser_Exception('Brak lewego argumentu dla modulo w miejscu '.$token->getPosition().'!');
			}
			...
			if (!isset($tokens[$key+1])) {
				throw new Parser_Exception('Brak prawego argumentu dla modulo w miejscu '.$token->getPosition().'!');
			}
